...
+(BOOL)deleteWhere:(NSString *)where{
    ...
    //遍历模型对象
    for (CoreFMDBModel *fmdbModel in deleteModels) {
        
        [fmdbModel.class enumerateIvarsWithBlock:^(MJIvar *ivar, BOOL *stop) {
            ...
            if(!skip){
                ...
                id value =[fmdbModel valueForKeyPath:ivar.propertyName];
                ...
                if([sqliteTye isEqualToString:EmptyString]){//模型字段
                    ...
                    NSString *where=[NSString stringWithFormat:@"pModel='%@' AND pid=%@",NSStringFromClass(fmdbModel.class),@(fmdbModel.hostID)];
